...
public class InversoController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) { 
        ...
        il = new InversoLogica();
        
        this.base = pc.getBase();
        il.setBase(this.base);
    ...
    public void escribirHistorial(){String op = tfOp1.getText();
        String mod = tfModulo.getText();
        String resul = tfResultado.getText();

        String historial = ("inv( " + op + " , " + mod + " ) = " + resul + "\n\n");
    ...
    private void error(String t){
        ...
        datos.setTitle("Valores incorrectos");
        ...
        if(enc)
        {
            ...
            if(enc) // Valores correctos
            {
                ...
                if(enc) // Números enteros
                {
                    ...
                    if(enc)
                    {
                        MCDLogica mcd = new MCDLogica();
                        mcd.setBase(this.base);
                        enc = (mcd.mcd(op, mod).equals("1"));
                        if(enc)
                        {
                            resul = il.inverso(op, mod);
                        ...
                        else
                        {
                            ...
                            // op1 y op2 (mod) tienen que ser primos primos relativos, es decir, op1 mod op2 = 1 == mcd(op1, op2) = 1
                            error("Para calcular el inverso de un número p en módulo n, "
                                    + "es necesario que p y n sean primos relativos, es decir, mcd(p, n) = 1");
                            tfResultado.setText("");
                        }
                    }
                    else
                    ...
                    error("No se puede realizar el inverso de números decimales.");
            }
            else
