    ...
    private void resultado(ActionEvent event) {
        ...
        if(enc)
        {
            ...
            if(enc) // Valores correctos
            {
                ...
                else // Binario o hexadecimal
                {
                    BigInteger num1 = new BigInteger(op1, this.base);
                    BigInteger num2 = new BigInteger(op2, this.base);
                    if(num1.compareTo(num2) < 0) // El resultado sería negativo
                        enc = false;
                }
                if(auxModulo() && enc) // Módulo activo
                {
                    ...
                    if(enc) // Números enteros
                    {
                        ...
                        if(enc)
                        {
                            ...
                            if(enc)
                            {
                                ...
                                {
                                    resul = rl.restaModulo(op1, op2, mod);
                                    mod = validar.separadorMiles(mod, this.base);
                ...
                {
                    resul = rl.restaDecimal(op1, op2);
                    op1 = validar.puntoAComa(op1);
                    op2 = validar.puntoAComa(op2);
                    resul = validar.puntoAComa(resul);
                }
                else if(enc)// Módulo inactivo base 2 y 16
                    resul = rl.restaEnteros(op1, op2);
                else
                    error("Operando 1 < Operando 2. \nEl resultado no puede ser negativo al trabajar en binario o hexadecimal.");
            }
            else
